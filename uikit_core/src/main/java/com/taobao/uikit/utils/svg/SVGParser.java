 ...
 */
/**
 * Entry point for parsing SVG files for Android. Use one of the various static methods for parsing SVGs by resource, asset or input stream. Optionally, a single color can be searched and replaced in
 * the SVG while parsing. You can also parse an svg path directly.
 *
...
public class SVGParser
{
     ...
     * @throws SVGParseException if there is an error while parsing.
     */
    public static SVG getSVGFromInputStream(InputStream svgData, int searchColor, int replaceColor, boolean pathOnly) throws SVGParseException
    {
        return SVGParser.parse(svgData, searchColor, replaceColor, false, pathOnly);
    }
     ...
     * @throws SVGParseException if there is an error while parsing.
     */
    public static SVG getSVGFromString(String svgData, int searchColor, int replaceColor, boolean pathOnly) throws SVGParseException
    {
        return SVGParser.parse(new ByteArrayInputStream(svgData.getBytes()), searchColor, replaceColor, false, pathOnly);
    }
     ...
     * @throws SVGParseException if there is an error while parsing.
     */
    public static SVG getSVGFromResource(Resources resources, int resId, int searchColor, int replaceColor, boolean pathOnly) throws SVGParseException
    {
        return SVGParser.parse(resources.openRawResource(resId), searchColor, replaceColor, false, pathOnly);
    }
     ...
     * @throws java.io.IOException if there was a problem reading the file.
     */
    public static SVG getSVGFromAsset(AssetManager assetMngr, String svgPath, int searchColor, int replaceColor, boolean pathOnly) throws SVGParseException, IOException
    {
        InputStream inputStream = assetMngr.open(svgPath);
        SVG svg = getSVGFromInputStream(inputStream, searchColor, replaceColor, pathOnly);
        inputStream.close();
        return svg;
    }
    ...
    private static SVG parse(InputStream in, Integer searchColor, Integer replaceColor, boolean whiteMode, boolean pathOnly) throws SVGParseException
    {
        Log.d("yanpei", "Parsing SVG...");
        try
        {
            ...
            Log.d("yanpei", "Parsing complete in " + (System.currentTimeMillis() - start) + " millis.");
            SVG result = new SVG(picture, handler.bounds, handler.viewBox, handler.paths);
    ...
    /**
     * This is where the hard-to-parse paths are handled. Uppercase rules are absolute positions, lowercase are relative. Types of path rules: <p/> <ol> <li>M/m - (x y)+ - Move to (without drawing)
     * <li>Z/z - (no params) - Close path (back to starting point) <li>L/l - (x y)+ - Line to <li>H/h - x+ - Horizontal ine to <li>V/v - y+ - Vertical line to <li>C/c - (x1 y1 x2 y2 x y)+ - Cubic
     * bezier to <li>S/s - (x2 y2 x y)+ - Smooth cubic bezier to (shorthand that assumes the x2, y2 from previous C/S is the x1, y1 of this bezier) <li>Q/q - (x1 y1 x y)+ - Quadratic bezier to <li>T/t
     * - (x y)+ - Smooth quadratic bezier to (assumes previous control point is "reflection" of last one w.r.t. to current point) </ol> <p/> Numbers are separate by whitespace, comma or nothing at all
     * (!) if they are self-delimiting, (ie. begin with a - sign)
     *
    ...
    private static class SVGHandler extends DefaultHandler
    {
        ...
        public void startElement(String namespaceURI, String localName, String qName, Attributes atts) throws SAXException
        {
            ...
            if (localName.equals("svg"))
            {
                ...
                viewBox = new RectF(0, 0, width, height);
                canvas = picture.beginRecording(width, height);
            }
            else if (!pathOnly && localName.equals("defs"))
            {
                // Ignore
            }
            ...
            else if (!pathOnly && !hidden && localName.equals("rect"))
            {
                Float x = getFloatAttr("x", atts);
                if (x == null)
                {
            ...
            else if (!pathOnly && !hidden && localName.equals("line"))
            {
            ...
            else if (!pathOnly && !hidden && localName.equals("circle"))
            {
            ...
            else if (!pathOnly && !hidden && localName.equals("ellipse"))
            {
                Float centerX = getFloatAttr("cx", atts);
                Float centerY = getFloatAttr("cy", atts);
