...
/**
 * 
 * TButton: UIKit's custom Button
 * 
 * @author jiajing
 * 
 */
public class TButton extends Button implements ViewHelper, IFeatureList<Button> {
	private FeatureList<Button> mFeatureList = new FeatureList<Button>(this);
	...
	protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
		for (AbsFeature<? super Button> feature : mFeatureList) {
		...
		for (int i = mFeatureList.size() - 1; i >= 0; i--) {
			AbsFeature<? super Button> feature = mFeatureList.get(i);
			if (feature instanceof MeasureCallback) {
	...
	@Override
	public AbsFeature<? super Button> findFeature(
	...
	@Override
	public boolean addFeature(AbsFeature<? super Button> feature) {
