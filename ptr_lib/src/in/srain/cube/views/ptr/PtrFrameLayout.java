...
import in.srain.cube.views.ptr.util.NetWorkUtil;
...
public class PtrFrameLayout extends ViewGroup {
    ...
    public PtrFrameLayout(Context context, AttributeSet attrs, int defStyle) {
        ...
        if (arr != null) {
            ...
            mEmptyId = arr.getResourceId(R.styleable.PtrFrameLayout_ptr_empty, mEmptyId);
    ...
    protected void onFinishInflate() {
        ...
        if (childCount > 3) {
            ...
            if (mEmptyId != 0 && mEmptyView == null) {
                mEmptyView = findViewById(mEmptyId);
            }
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        if (mEmptyView != null) {
            measureContentView(mEmptyView, widthMeasureSpec, heightMeasureSpec);
            if (DEBUG && DEBUG_LAYOUT) {
                ViewGroup.MarginLayoutParams lp = (MarginLayoutParams) mEmptyView.getLayoutParams();
                PtrCLog.d(LOG_TAG, "onMeasure content, width: %s, height: %s, margin: %s %s %s %s",
                        getMeasuredWidth(), getMeasuredHeight(),
                        lp.leftMargin, lp.topMargin, lp.rightMargin, lp.bottomMargin);
                PtrCLog.d(LOG_TAG, "onMeasure, currentPos: %s, lastPos: %s, top: %s",
    ...
    private void layoutChildren() {
        ...
        if (mEmptyView != null) {
            ...
            MarginLayoutParams lp = (MarginLayoutParams) mEmptyView.getLayoutParams();
            final int left = paddingLeft + lp.leftMargin;
            final int top = paddingTop + lp.topMargin + offsetX;
            final int right = left + mEmptyView.getMeasuredWidth();
            final int bottom = top + mEmptyView.getMeasuredHeight();
            if (DEBUG && DEBUG_LAYOUT) {
                PtrCLog.d(LOG_TAG, "onLayout content: %s %s %s %s", left, top, right, bottom);
            }
    ...
    //UI开启刷新模式
    public void setStartRefresh(int x) {
        mScrollChecker.tryToScrollTo(x, 2000);
        performRefresh();
    }
}
