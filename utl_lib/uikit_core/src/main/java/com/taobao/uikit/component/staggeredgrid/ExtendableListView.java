...
/**
 * An extendable implementation of the Android {@link android.widget.ListView} <p/> This is partly inspired by the incomplete StaggeredGridView supplied in the Android 4.2+ source & the {@link
 * android.widget.AbsListView} & {@link android.widget.ListView} source; however this is intended to have a smaller simplified scope of functionality and hopefully therefore be a workable solution.
 * <p/> Some things that this doesn't support (yet) - Dividers (We don't use them in our Etsy grid) - Edge effect - Fading edge - yuck - Item selection - Focus <p/> Note: we only really extend {@link
 * android.widget.AbsListView} so we can appear to be one of its direct subclasses. However most of the code we need to modify is either 1. hidden or 2. package private So a lot of it's code and some
 * {@link android.widget.AdapterView} code is repeated here Be careful with this - not everything may be how you expect if you assume this to be a regular old {@link android.widget.ListView}
 */
@Deprecated
public abstract class ExtendableListView extends AbsListView
{
    ...
    /**
     * Add a fixed view to appear at the top of the list. If addHeaderView is called more than once, the views will appear in the order they were added. Views added using this call can take focus if
     * they want. <p/> NOTE: Call this before calling setAdapter. This is so ListView can wrap the supplied cursor with one that will also account for header and footer views.
     *
    ...
    /**
     * Add a fixed view to appear at the bottom of the list. If addFooterView is called more than once, the views will appear in the order they were added. Views added using this call can take focus
     * if they want. <p/> NOTE: Call this before calling setAdapter. This is so ListView can wrap the supplied cursor with one that will also account for header and footer views.
     *
     * @param v            The view to add.
     * @param data         Data to associate with this view
    ...
    protected void layoutChildren()
    {
        ...
        try
        {
            ...
            {
                throw new IllegalStateException("The content of the adapter has changed but " + "ExtendableListView did not receive a notification. Make sure the content of " +
                                                "your adapter is not modified from a background thread, but only " + "from the UI thread. [in ExtendableListView(" + getId() + ", " + getClass() +
    ...
    /**
     * Get a view and have it show the data associated with the specified position. This is called when we have already discovered that the view is not available for reuse in the recycle bin. The only
     * choices left are converting an old view or making a new one.
     *
     ...
     */
    protected void onLayoutChild(final View child, final int position, final boolean flowDown, final int childrenLeft, final int childTop, final int childRight, final int childBottom)
    {
     ...
     */
    protected void onOffsetChild(final View child, final int position, final boolean flowDown, final int childrenLeft, final int childTop)
    {
    ...
    /**
     * Responsible for fling behavior. Use {@link #start(int)} to initiate a fling. Each frame of the fling is handled in {@link #run()}. A FlingRunnable will keep re-posting itself until the fling is
     * done.
     */
    ...
    /**
     * Update the status of the list based on the empty parameter.  If empty is true and we have an empty view, display it.  In all the other cases, make sure that the listview is VISIBLE and that the
     * empty view is GONE (if it's not null).
     */
    ...
    /**
     * Re-implementing some properties in {@link android.view.ViewGroup.LayoutParams} since they're package private but we want to appear to be an extension of the existing class.
     */
    ...
    class RecycleBin
    {
        ...
        /**
         * Views that were on screen at the start of layout. This array is populated at the start of layout, and at the end of layout all view in mActiveViews are moved to mScrapViews. Views in
         * mActiveViews represent a contiguous range of Views, with position of the first view store in mFirstActivePosition.
         */
        ...
        public void setViewTypeCount(int viewTypeCount)
        {
            ...
            @SuppressWarnings("unchecked") ArrayList<View>[] scrapViews = new ArrayList[viewTypeCount];
    ...
    public static class ListSavedState extends ClassLoaderSavedState
    {
        ...
        {
            return "ExtendableListView.ListSavedState{" + Integer.toHexString(System.identityHashCode(this)) + " selectedId=" + selectedId + " firstId=" + firstId + " viewTop=" + viewTop +
